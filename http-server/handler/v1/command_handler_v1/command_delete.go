...
func (h *Handler) DeleteCommandHandler_V1(w http.ResponseWriter, r *http.Request) error {
	ctx := r.Context()
	...
	if err != nil {
		logger.Error("DeleteCommandHandler_V1 req={%s}: %s", ctx.Value("XREQID").(string), err.Error())
		return httperr.BadRequest(tr.TErr("invalid-command-id"))
	}

	if err := h.Store.DeviceCommands.Delete_DeviceCommandByUuidAndId(ctx, authToken.User.UserContact.UserUUID, uint64(commandId)); err != nil {
		return httperr.Db(ctx, err)
	}

	httpx.HttpResponse(w, r, http.StatusOK, tr.T("command-deleted-successfully"))
	return nil
}
