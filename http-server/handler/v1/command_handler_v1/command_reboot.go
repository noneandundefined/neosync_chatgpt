...
func (h *Handler) CommandRebootHandler_V1(w http.ResponseWriter, r *http.Request) error {
	ctx := r.Context()
	...
	rabbitData := gtypes.RabbitMQ_TransitBinary{
		...
		Imei:  imei,
		NResp: true,
		Data:  []byte(constants.REBOOT_COMMAND),
	}

	if err := h.RMQ.SendToRabbitAsync(ctx, rabbitData); err != nil {
		logger.Error("CommandRebootHandler_V1 req={%s}: failed to send to RabbitMQ: %s", ctx.Value("XREQID").(string), err.Error())
		return httperr.Conflict(tr.TErr("rabbitmq-publish-failed"))
	}

	httpx.HttpResponse(w, r, http.StatusOK, tr.T("device-reboot-wait"))
	return nil
}
