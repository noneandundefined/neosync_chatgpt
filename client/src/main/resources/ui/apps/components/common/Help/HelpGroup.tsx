...
import { HELP_GROUPs_CONTROLs, HELP_GROUPs_STEPs } from '@/constants/Help.constant';
...
const HelpGroup: React.FC<HelpGroupProps> = ({ title, description, changeSection }) => {
	...
	return (
		<div className="space-y-5">
			...
			<div className="flex flex-col lg:flex-row items-stretch gap-5">
				<div>
					<img src="/local/templates/neosync/help/gui-groups.png" alt={t('message.help-group-image-alt')} draggable={false} />
				</div>
			...
			<div className="space-y-3">
				...
				<div className="bg-white border rounded-xl overflow-hidden">
					{HELP_GROUPs_CONTROLs.map((item, index) => {
						const Icon = item.icon;

						return (
							<div key={item.titleKey} className={`grid lg:grid-cols-[50px_280px_1fr] gap-3 px-2 py-3 lg:items-center ${index !== HELP_GROUPs_CONTROLs.length - 1 ? 'border-b' : ''}`}>
